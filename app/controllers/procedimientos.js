...
/**
    ...
    rootPath = path.normalize(rootPath + '/public/');
    procPath = path.normalize(rootPath + '/contenido/');
    pdfPath = path.normalize(rootPath + '/pdfs/');
    docPath = path.normalize(pdfPath + nombrePdf);
    url = '/pdfs/' + nombrePdf;
    ...
    crearPdf(req,res, function(rutaArchivo) {
        var message = {
            text: 'Distinguido ' + destinatario.nombre + ',\n' + req.user.name + ' le ha enviado el manual ' +
                proc.nombre + ' ' + comentario.replace('<p>', '').replace('</p>','\n') + ' \n*****favor no responder este correo*****', // plaintext body, // plaintext body
